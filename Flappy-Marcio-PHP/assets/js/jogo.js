...
const imagens = {
  ...
  passaro: [
    carregarImagem("assets/imgs/1.png"),
    carregarImagem("assets/imgs/2.png"),
    carregarImagem("assets/imgs/3.png"),
  ],
  marcio: Array.from({ length: 30 }, (_, i) =>
    carregarImagem(`marcioimg/frame-${String(i + 1).padStart(2, "0")}.gif`),
  ),
};
...
function velocidadeAtual() {
  ...
  const base =
    Number(config.velocidade_inicial || 3.85) * Number(modo.vel || 1);
...
function aberturaAtual() {
  const modo = modoAtual();
  const inicial =
    Number(config.abertura_inicial || 218) + Number(modo.abertura || 0);
  const minima =
    Number(config.abertura_minima || 178) + Number(modo.abertura || 0) * 0.5;
...
async function enviarAcesso(acao, usuario, senha) {
  ...
  const resposta = await fetch("php/ranking.php", {
    method: "POST",
    body: dados,
  });
...
async function iniciarPartidaServidor() {
  ...
  const dados = new FormData();
  dados.append("acao", "iniciar_partida");
...
async function salvarPontuacao() {
  ...
  try {
    ...
    const todas = await fetch("php/ranking.php?acao=jogador")
      .then((r) => r.json())
      .catch(() => null);
...
async function carregarRanking() {
  ...
  try {
    ...
    const resposta = await fetch(
      `php/ranking.php?acao=listar&periodo=${periodo}&dificuldade=${dificuldadeFiltro}`,
    );
    const retorno = await resposta.json();
    ...
    document.getElementById("ranking").innerHTML =
      '<li class="vazio">Ative o MySQL e abra o setup.php. Sem banco, nem o ranking acredita.</li>';
  } finally {
...
function prepararTela() {
  ...
  if (btnApresentacao) {
    btnApresentacao.addEventListener("click", () => {
      ...
      btnApresentacao.textContent = document.body.classList.contains(
        "modo-apresentacao",
      )
